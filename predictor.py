...
# --- HeteroGNN_GAT_For_Embeddings Class Definition ---
...
def load_artifacts():
    ...
    try:
        ...
        else: print(f"CRITICAL WARNING: User-Card map NOT FOUND at {USER_TO_CARDS_MAPPING_PATH}."); user_to_cards_map_loaded = {}; artifacts_ok = False
    ...
    if not artifacts_ok: print("CRITICAL: One or more essential artifacts failed to load.")
...
def preprocess_transaction_for_gnn(transaction_data_dict_raw):
    ...
    df_tx['AmountToAvgRatioUser'] = df_tx['Amount'].iloc[0] / (avg_amt + 1e-6) if avg_amt !=0 else 0.0 # Avoid division by zero slightly differently
    ...
    if cat_le_dict_loaded:
        ...
        for col in TX_CATEGORICAL_FEATS_GNN: df_tx[col] = 0 # Fallback if LE dict not loaded
    ...
    user_val_raw = str(df_tx['User'].iloc[0]) if 'User' in df_tx else "UNKNOWN_USER"
    merchant_val_raw = str(df_tx['Merchant Name'].iloc[0]) if 'Merchant Name' in df_tx else "UNKNOWN_MERCHANT"
    user_idx, merchant_idx = 0, 0 # Default
    if user_le_loaded:
        try:
...
def predict_fraud(transaction_data_dict):
    ...
    card_validation_passed = False
    card_val_status_msg = "Card_Not_Validated_Or_Map_Missing"

    if user_to_cards_map_loaded: # Ensure map is not None and not empty
        if user_id_input in user_to_cards_map_loaded:
            ...
            if card_id_input in user_known_cards:
                print(f"PREDICTOR DEBUG: Card '{card_id_input}' IS IN known cards. Proceeding.")
                card_validation_passed = True
                card_val_status_msg = "Validated"
            else:
                ...
                return {
                    ...
                    "user_provided": user_id_input, "card_provided": card_id_input,
                }, 400
        else:
            print(f"PREDICTOR DEBUG: User '{user_id_input}' not found in User-Card mapping. Proceeding with prediction.")
            card_val_status_msg = "User_Not_In_Map_Proceeding"
            card_validation_passed = True # Allow prediction for new users for now
    else:
        print("PREDICTOR DEBUG: User-Card mapping not loaded or empty. Skipping card validation logic. Proceeding.")
        card_validation_passed = True # Allow prediction if map is missing

    # If card_validation_passed is False here, it means an error was returned above.
    # This check is redundant if the error path always returns.
    # if not card_validation_passed: # This block would only be hit if the above didn't return
    #     return {"error": "Card validation failed."}, 400
    ...
    try:
        ...
        print(f"Error during preprocessing for prediction: {e_preprocess}"); traceback.print_exc()
    ...
    # Only add edges if GNN model is actually loaded and indices are valid (for GNNs that need them)
    if gnn_model_loaded:
        if user_idx < gnn_model_loaded.user_emb.num_embeddings :
        ...
        if merchant_idx < gnn_model_loaded.merchant_emb.num_embeddings:
    ...
    try:
        ...
        print(f"Error during XGBoost prediction: {e_xgb}"); traceback.print_exc()
    ...
    return {
        ...
        "card_validation_status": card_val_status_msg
    }, 200
...
if __name__ == '__main__':
    ...
    if not artifacts_are_ok or not all([gnn_model_loaded, xgb_model_loaded, scaler_gnn_loaded, 
                ...
                user_to_cards_map_loaded is not None]):
        print("CRITICAL: Not all essential artifacts loaded. Exiting test.")
    else:
        if not user_to_cards_map_loaded: # Check if empty even if not None
            print("WARNING: User-Card mapping is empty. Card validation tests may not be meaningful.")
        ...
        known_user_for_test = '0'
        known_card_for_user_A = 'card_A_for_user_0'
        
        sample_tx_valid = {
            'User': known_user_for_test, 'Card': known_card_for_user_A, 
            'Year': 2023, 'Month': 10, 'Day': 26, 'Time': '12:30:45',
            'Amount': "$50.00", 'Use Chip': 'Chip Transaction', 
            'Merchant Name': '12345', 'Merchant City': 'NEW YORK', 'Merchant State': 'NY', 
            'Zip': '10001', 'MCC': '5411', 'Errors?': 'No Error', 
            'TransactionID': "test_tx_valid_card"
        }
        ...
        result_dict, status_code = predict_fraud(sample_tx_valid)
        print(f"Status: {status_code}, Prediction: {result_dict}")

        if known_user_for_test in user_to_cards_map_loaded:
            sample_tx_invalid_card = sample_tx_valid.copy()
            sample_tx_invalid_card['Card'] = 'WRONG_CARD_XYZ'
            sample_tx_invalid_card['TransactionID'] = "test_tx_invalid_card"
            print("\n--- Test Case 2: User with WRONG Card ---")
            result_dict, status_code = predict_fraud(sample_tx_invalid_card)
            print(f"Status: {status_code}, Prediction: {result_dict}")

        sample_tx_new_user = sample_tx_valid.copy()
        sample_tx_new_user['User'] = 'NEW_USER_TOTALLY_UNKNOWN'
        sample_tx_new_user['Card'] = 'CARD_FOR_NEW_USER_456'
        sample_tx_new_user['TransactionID'] = "test_tx_new_user"
        print("\n--- Test Case 3: NEW User (not in mapping) ---")
        result_dict, status_code = predict_fraud(sample_tx_new_user)
        print(f"Status: {status_code}, Prediction: {result_dict}")
