...
# --- Configuration ---
ARTIFACTS_DIR = "./" # Assume artifacts are in the same directory
...
# GNN Parameters (Ensure these match the GNN training config)
GNN_HIDDEN_CHANNELS = 64
GNN_OUT_CHANNELS_EMB = GNN_HIDDEN_CHANNELS
GNN_CLASSIFICATION_OUT_CHANNELS = 2
GNN_NUM_LAYERS = 2             # Ensure integer
GNN_GAT_HEADS = 2              # Ensure integer
GNN_DROPOUT_RATE_TRAINING = 0.3 # Dropout used during GNN model saving

# Feature names for GNN input
...
class HeteroGNN_GAT_For_Embeddings(torch.nn.Module):
    def __init__(self, hidden_channels, emb_out_channels,
                 ...
                 num_tx_node_features, dropout_rate, num_gnn_layers=2, gat_heads=4, # Defaults if not passed
                 classification_out_channels=None):
        super().__init__()
        self.dropout_rate = dropout_rate
        self.num_gnn_layers = int(num_gnn_layers) # Ensure integer
        self.gat_heads = int(gat_heads)           # Ensure integer
        ...
        for i in range(self.num_gnn_layers): # This now uses an integer
            conv_dict_layer = {
                key_type: GATConv(
                    (-1, -1), hidden_channels // self.gat_heads, # Ensure gat_heads is not zero
                    heads=self.gat_heads, # This now uses an integer
    ...
    def forward(self, x_dict, edge_index_dict, output_type='embeddings'):
        ...
        if 'transaction' in x_dict and x_dict['transaction'] is not None:
            if self.has_tx_features and x_dict['transaction'].shape[1] > 0 :
                ...
                if num_current_tx_nodes > 0: x_dict_transformed['transaction'] = self.tx_base_emb.repeat(num_current_tx_nodes, 1)
            elif x_dict['transaction'].shape[1] == 0 and self.has_tx_features: # Case where tx_lin exists but no features passed
                 num_current_tx_nodes = x_dict['transaction'].shape[0]
                 ...
                 elif num_current_tx_nodes > 0: # Fallback if tx_lin is problematic
...
def load_artifacts():
    ...
    try:
        ...
        else: print(f"CRITICAL WARNING: User-Card map NOT FOUND at {USER_TO_CARDS_MAPPING_PATH}."); user_to_cards_map_loaded = {}; artifacts_ok = False # Essential
    ...
    try:
        gnn_model_loaded = HeteroGNN_GAT_For_Embeddings(
            ...
            dropout_rate=GNN_DROPOUT_RATE_TRAINING, # Use the rate it was saved with
            num_gnn_layers=int(GNN_NUM_LAYERS),     # Ensure integer
            gat_heads=int(GNN_GAT_HEADS)            # Ensure integer
        ).to(device)
        if os.path.exists(GNN_MODEL_PATH):
            gnn_model_loaded.load_state_dict(torch.load(GNN_MODEL_PATH, map_location=device))
            gnn_model_loaded.eval() # Set to eval mode
    ...
    if os.path.exists(OPTIMAL_THRESHOLD_XGB_LOAD_PATH):
        ...
        print(f"   WARNING: Optimal threshold file NOT FOUND at {OPTIMAL_THRESHOLD_XGB_LOAD_PATH}. Using default 0.5 (This might lead to poor classification if default is not good).")
        optimal_threshold_loaded = 0.5
...
def preprocess_transaction_for_gnn(transaction_data_dict_raw):
    ...
    if user_le_loaded and hasattr(user_le_loaded, 'classes_'):
        try:
            ...
            elif len(user_le_loaded.classes_) > 0 : user_idx = 0 # Map to first known if unseen
    ...
    if merchant_le_loaded and hasattr(merchant_le_loaded, 'classes_'):
        try:
            ...
            elif len(merchant_le_loaded.classes_) > 0 : merchant_idx = 0 # Map to first known if unseen
...
def predict_fraud(transaction_data_dict):
    ...
    if user_to_cards_map_loaded and isinstance(user_to_cards_map_loaded, dict): # Ensure it's a dict
        ...
        elif user_to_cards_map_loaded: # User not in map, but map exists
            print(f"PREDICTOR DEBUG: User '{user_id_input}' not found in User-Card mapping. Proceeding.")
            card_val_status_msg = "User_Not_In_Map_Proceeding"
        # If user_to_cards_map_loaded is empty, the outer else handles it
    else: # user_to_cards_map_loaded is None or not a dict (e.g. failed to load)
        print("PREDICTOR DEBUG: User-Card mapping not loaded or empty. Skipping card validation. Proceeding.")
        card_val_status_msg = "Map_Missing_Or_Empty_Proceeding"
